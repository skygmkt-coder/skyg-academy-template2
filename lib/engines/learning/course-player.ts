...
async function getCourseRow(courseId: string): Promise<CourseRow | null> {
  ...
  const { data, error } = await supabase.from("courses").select(courseColumns).eq("id", courseId).maybeSingle();
...
async function listResources(courseId: string): Promise<ResourceRow[]> {
  ...
  const supabase = await createClient();
  const { data, error } = await supabase.from("lesson_resources").select(resourceColumns).in("lesson_id", lessons).order("display_order", { ascending: true });
...
function mapLesson(row: LessonRow, resources: ResourceRow[]): Omit<CoursePlayerLesson, "isCompleted"> {
  const mediaUrl = row.media_bucket && row.media_path ? protectedMediaUrl({ bucket: row.media_bucket, path: row.media_path, courseId: row.product_id, lessonId: row.id }) : row.video_url;
  return {
    id: row.id,
    ...
    mediaKind: row.media_kind === "pdf" || row.media_kind === "image" || row.media_kind === "external" ? row.media_kind : row.media_kind === "video" ? "video" : null,
    ...
    resources: resources.map((resource) => ({ id: resource.id, lessonId: resource.lesson_id, title: resource.title, fileUrl: resource.file_bucket && resource.file_path ? protectedMediaUrl({ bucket: resource.file_bucket, path: resource.file_path, courseId: row.product_id, lessonId: resource.lesson_id }) : resource.file_url, displayOrder: resource.display_order }))
  };
}
...
function mapCourse(course: CourseRow): CoursePlayerExperience["course"] {
  return { id: course.id, creatorId: course.creator_id, title: course.title, slug: course.slug, description: course.description, coverImageUrl: course.cover_image_url, coverImagePath: course.cover_image_path, thumbnailUrl: course.thumbnail_url, thumbnailPath: course.thumbnail_path, isPublished: course.is_published, createdAt: course.created_at, updatedAt: course.updated_at };
}
