...
import { PaymentProofPreview } from "@/components/commerce/payment-proof-preview";
          ...
          {payments.map((payment) => (
            <article key={payment.id} className="space-y-4 rounded-lg border border-line-subtle bg-surface-base p-4 shadow-soft">
              <div className="grid gap-4 md:grid-cols-[1fr_16rem]">
                ...
                {payment.proofUrl && proofUrls.get(payment.id) ? (
                  <PaymentProofPreview
                    imageUrl={proofUrls.get(payment.id) ?? ""}
                    label={payment.order.student?.email ?? payment.order.userId}
                  />
                ) : (
                  <div className="rounded-md border border-dashed border-line-strong bg-surface-muted p-3 text-sm text-ink-muted">
